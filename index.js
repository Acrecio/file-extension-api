...
app.use(route.get('/v1/files', function*(){
  this.body = yield db.all("SELECT * FROM Files")
}))

app.use(route.get('/v1/names', function*(){
  this.body = yield db.all("SELECT name FROM Files")
}))

app.use(route.get('/v1/classifications', function*(){
...
app.use(route.get('/v1/search/company/:company', function*(company){
  this.body = yield db.all("SELECT * FROM Files WHERE company LIKE ?", `%${company}%`)
}))

app.use(route.get('/v1/search/association/:association', function*(association){
  this.body = yield db.all("SELECT * FROM Files WHERE association LIKE ?", `%${association}%`)
}))

app.use(route.get('/v1/search/classification/:classification', function*(classification){
